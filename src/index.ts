...
const main = async function () {
  ...
  // Make array
  const keywords: string[] = keywordsRead.toString().replace(/ /g, '').replace(/:/g, '').split('\r\n')
  const pageTypes: string[] = pageTypeRead.toString().replace(/ /g, '').replace(/:/g, '').split('\r\n')
  const pageFormats: string[] = pageFormatRead.toString().replace(/ /g, '').replace(/:/g, '').split('\r\n')

  // Make string with all dorks
  let strDorks: string = 'Dorks generated by DORKS-GEN-TS (use this with [ inurl: ])\r\n'

  // [keyword][page_type][page_format] => [minecraft][.php?][userid=]
  keywords.forEach(keyword => {
    pageTypes.forEach(pagetype => {
      pageFormats.forEach(pageformat => {
        strDorks += `\r\n${keyword}${pagetype}${pageformat}`
      });
    });
  });

  const rows: number = strDorks.match(/\r\n/g).length - 1
