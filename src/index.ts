...
const main = async function () {
  ...
  for (let i = 0; i < keywords.length; i++) {
    if (keywords[i].replace(/ /g, '') === '') keywords.slice(i, 1)
  }
  for (let i = 0; i < pageTypes.length; i++) {
    if (pageTypes[i].replace(/ /g, '') === '') pageTypes.slice(i, 1)
  }
  for (let i = 0; i < pageFormats.length; i++) {
    if (pageFormats[i].replace(/ /g, '') === '') pageFormats.slice(i, 1)
  }
